...
Input: two integers, count (>=0) and start (any integer)
Output: list with length of count and value % start == 0


Rules:

    Implicit:
        Each element should equal (its index + 1) * start
...
def sequence(count:int, start:int)->list:
    result = []
    for index in range(count):
        result.append((index + 1) * start)
    return result
